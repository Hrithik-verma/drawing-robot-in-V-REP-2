...
if (clientID>-1)
       ...
       [~,dum]= sim.simxGetObjectHandle(clientID,'IRB140_target',sim.simx_opmode_blocking);
        RGB = imread('jerry.jpg');                                                            %image input to convert to binay image 
...
% Create mask based on chosen histogram thresholds
    ...
    for m= 1:length(x)
    [returnCode]=sim.simxSetObjectPosition(clientID,dum,-1,[-0.22+(x(m)*0.0003),-0.175+(y(m)*0.0003),(z(m)*0.004)+0.515],sim.simx_opmode_blocking);
    end
